<template>
  <div class="app-container" v-loading="loading">
    ...
    <el-table
      ...
      <el-table-column
        ...
        <template slot-scope="scope">
          ...
          <el-button type="text" @click="delDistribute(scope.row)" :disabled="scope.row.status===3">删除</el-button>
          <!-- <el-button type="text" @click="subAudit(scope.row.id,2)" :loading="btnLoading"
                     :disabled="scope.row.status!=6&&scope.row.status!=1">提交审核
          </el-button> -->
    ...
    :close-on-click-modal="false"
      ...
      <el-form :model="newMemberForm" :rules="Rules" ref="addForm" :label-width="newFormLabelWidth"
               size="mini">
        ...
        <el-form-item label="当前分佣模式：" prop="commissionModel">
          <el-select v-model="newMemberForm.commissionModel" :disabled="true" placeholder="当前分佣模式">
            <el-option
        ...
        <el-form-item label="抽佣比例:" show-message prop="payProrata" v-show="newMemberForm.commissionModel==1">
          ...
          %
          <el-tooltip class="item" effect="dark" :content="`${subName}的分佣比例，比如填写60%，则佣金分成60%给${subName}`"
                      placement="top-start">
    ...
    <el-button type="primary" @click="subAddBefore(1)" :loading="btnLoading">确定</el-button>
  </span>
    </el-dialog>
    <!--新增模态框-->
    ...
    :close-on-click-modal="false"
      ...
      <el-form :model="editMemberForm" :rules="Rules" ref="editMemberForm" :label-width="newFormLabelWidth"
        ...
        <el-form-item label="联系人：" show-message prop="contact">
          <el-input v-model="editMemberForm.contact"
                    :disabled="currentDisabled!=1"></el-input>
        </el-form-item>
        <el-form-item label="省份：" prop="province">
        ...
        <el-form-item label="抽佣比例:" show-message prop="payProrata" v-show="editMemberForm.commissionModel==1">
          ...
          %
          <el-tooltip class="item" effect="dark" :content="`${disName}的分佣比例，比如填写60%，则佣金分成60%给${disName}`"
                      placement="top-start">
            <i class="el-icon-question"></i>
          </el-tooltip>
        </el-form-item>
...
<script>
  ...
  export default {
    ...
    data() {
      return {
        ...
        Rules: {
          ...
          city: [
            {required: true, message: '请选择城市/县', trigger: 'change'}
          ],
          address: [
    ...
    methods: {
      ...
      newMemberOpen: function () {
        ...
        getComModel().then(res => {
          let data = res.obj;
          this.newMemberForm.commissionModel = data + '';
    ...
    * */
       ...
       * 删除推广商
      * */
      delDistribute: function (data) {
        let oid = data.id; //eg :data.id
        this.$confirm(`确定删除${this.subName}吗？本次操作无法撤销`).then(() => {
          delSub(oid).then(res => {
            this.$message({
              message: res.msg,
              type: 'success',
      ...
      subAdd: function (status) {
        ...
        } else if (form.commissionModel == 1) {
          form.baseFeeRateDTOJson = ''; //比例模式，底价JSON置空
          form.payProrata /= 100
        } else {
          this.$message.error('没有选择模式');
          return
        }
      ...
      feeBackShow: function (form, showData) {
        ...
        if (showData.length != 0) {
          ...
          for (let i in form) {
            for (let j in form[i].selectItemList) {
              this.$set(form[i].selectItemList[j], 'feeRate', 0)
            }
          }
        }
