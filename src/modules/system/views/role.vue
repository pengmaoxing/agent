    ...
    <el-table
      ...
      <el-table-column label="公司类型">
        <template slot-scope="scope">
          <span v-if="scope.row.type===1">{{topName}}</span>
          <span v-else-if="scope.row.type===2">{{agentName}}</span>
          <span v-else-if="scope.row.type===3">{{disName}}</span>
          <span v-else-if="scope.row.type===4">{{subName}}</span>
        </template>
      </el-table-column>
