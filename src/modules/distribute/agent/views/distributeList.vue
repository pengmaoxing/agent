          ...
          <div v-for="(main,index) in newDynamicForm" class="box" :key="index">
            <div v-for="(item,index) in main.selectItemList" :key="index">
              <el-form-item :label="main.selectItem.name+'('+item.name+')'" label-width="160px">
                <el-input-number v-model="item.feeRate" :precision="2" :step="0.01" :min="0.00" :max="50.00" class="dymInput"></el-input-number>%
              </el-form-item>
            </div>
          </div>
        </el-form-item>
  ...
  export default {
    ...
    data() {
      return {
        ...
        levelOption: [{value: 1, name: '顶级渠道商'}, {value: 2, name: '普通渠道商'}], //合作级别
        //编辑
    ...
    * */
      ...
      * */
      delDistribute: function (data) {
        ...
        this.$confirm(`确定删除${this.disName}吗？本次操作无法撤销`)
          .then(_ => {
            delDistribute(oid).then(res => {
              this.$message({
                message: res.msg,
                type: 'success',
              });
