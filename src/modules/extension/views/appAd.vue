    ...
    :close-on-click-modal="false"
            ...
            <div>
              <el-form :model="addPutForm" ref="addPutForm" :rules="adRule" label-width="140px" size="medium">
                ...
                <el-form-item label="每千次展示费用：" v-if="adType==2 || adType==1">
                  <el-input v-model="addPutForm.showCost" :disabled="true" class="input-width">
                    <template slot="append">元</template>
                  </el-input>
                </el-form-item>
          ...
          <template>
            <el-table
              ...
              <el-table-column
                ...
                label="广告类型">
                <template slot-scope="scope">
                  <span v-if="scope.row.appModel==1">普通广告</span>
                  <span v-else>支付宝小程序大屏</span>
                </template>
              </el-table-column>
              <el-table-column
                prop="remark"
            ...
            <div>
              ...
              <template>
                <el-form ref="addAdForm" :model="addAdForm" :rules="adRule" size="medium" label-width="120px">
                  ...
                  <el-form-item label="广告类型：">
                    <el-radio-group v-model="addAdForm.appModel">
                      <el-radio :label="1">普通广告（APP首页内置广告）</el-radio>
                      <el-radio :label="2" style="margin-top: 5px">大屏（支付宝小程序营业模式广告）</el-radio>
                    </el-radio-group>
                  </el-form-item>
                  ...
                  <el-form-item label="广告图片：" v-if="addAdForm.type=='1' || addAdForm.type=='2' " prop="imageId">
                    <el-upload
                      ...
                      :headers="headers">
                      <img v-if="addPhotoUrl" :src="addPhotoUrl" :class="addAdForm.appModel==1?'logo':'logo-2'">
                      <i v-else class="el-icon-plus logo-uploader-icon"
                         :class="addAdForm.appModel==1?'logo-uploader-icon':'logo-uploader-icon-2'"></i>
                    </el-upload>
                    <span class="tips" style="position: absolute;top: 0;margin-left: 10px">
                      {{addAdForm.appModel==1?'建议上传375*80px':'建议上传800*1280px'}}
                      <br/>格式为jpg、png、jpeg的图片</span>
                  </el-form-item>
                  <!--注意有表单判断 v-if-->
            ...
            <div>
              ...
              <template>
                <el-form ref="editAdForm" :rules="adRule" :model="editAdForm" size="medium" label-width="120px">
                  ...
                  <el-form-item label="广告类型：">
                    <el-radio-group v-model="editAdForm.appModel">
                      <el-radio :label="1">普通广告（APP首页内置广告）</el-radio>
                      <el-radio :label="2" style="margin-top: 5px">大屏（支付宝小程序营业模式广告）</el-radio>
                    </el-radio-group>
                  </el-form-item>
                  ...
                  <el-form-item label="广告图片：" v-if="editAdForm.type=='1' || editAdForm.type=='2' " prop="imageId">
                    <el-upload
                      ...
                      :before-upload="beforeLogoUpload"
                      :headers="headers">
                      <img v-if="editPhotoUrl" :src="editPhotoUrl" :class="editAdForm.appModel==1?'logo':'logo-2'">
                      <i v-else class="el-icon-plus"
                         :class="editAdForm.appModel==1?'logo-uploader-icon':'logo-uploader-icon-2'"></i>
                    </el-upload>
                    <span class="tips" style="position: absolute;top: 0;margin-left: 10px">
                      {{editAdForm.appModel==1?'建议上传375*80px':'建议上传800*1280px'}}
                      <br/>格式为jpg、png、jpeg的图片</span>
                  </el-form-item>
                  <!--注意有表单判断 v-if-->
                  <el-form-item label="推广网址：" v-if="editAdForm.type=='1' || editAdForm.type=='3' " prop="imageLink">
                    <el-input v-model="editAdForm.imageLink" placeholder="输入广告跳转地址(必须携带http:// 或 https://)"
                              class="ad-input-width"></el-input>
                    <br/>
                    <span class="tips">跳转地址必须携带 “ http:// ” 或 “ https:// ”，请谨慎录入，上线后点击广告图片，将跳转此网站</span>
                  </el-form-item>
            ...
            <div>
              ...
              <el-form :model="addStrategyForm" :rules="adRule" ref="addStrategyForm" label-width="120px">
                <el-form-item label="策略名称：" prop="strategicName">
                  <el-input v-model.trim="addStrategyForm.strategicName" placeholder="输入策略名称"></el-input>
                </el-form-item>
  ...
  export default {
    ...
    methods: {
      ...
      putSeeStrategyDetailOpen: function (data) {
        let promise = getStrategicDetail(data.id).then(res => {
          let data = res.obj;
      ...
      editAdOpen: function (rowData) {
        ...
        getAdvertiseDetail(id).then(res => {
          ...
          this.$set(this.editAdForm, 'appModel', rowData.appModel || 1)
