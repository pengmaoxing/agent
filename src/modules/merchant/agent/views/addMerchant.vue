        ...
        <div>
          ...
          <el-form-item prop="address">
            <el-input v-model.trim="addForm.address" class="input-width"
                      placeholder="详细地址（精确到门牌号）"></el-input><span class="tips">请不要填写省市县(区)信息</span>
          </el-form-item>
...
<script>
  ...
  import {addMerchant, getMerchant, updateMerchant} from '../api/addMerchant'
  ...
  export default {
    ...
    methods: {
          ...
          if (valid) {
            addMerchant(form).then(res => {
              if (res.obj == null || res.obj === '') {
                ...
                this.$message.error('返回数据异常')
              } else {
                //将返回的id 送往 下一个页面 运渠配置支付通道
                let id = res.obj;
                this.$message.success(res.msg + '即将前往支付通道');
                setTimeout(() => {
                  this.submitSuccessToPaywayPage(id)
                }, 1500)
              }
            }).catch(e => {
              this.btnLoading = false;
       ...
       * 保存后前往初选支付通道页面
       */
      submitSuccessToPaywayPage: function (id) {
        //前往支付通道预配置页面
        this.$router.push(
          {
            path: '/merchant/agent/views/payChannelList',
            query: {id: id, mode: 'jump'}
          }
        )
      },
      /**
      ...
      autoInputData: function (data) {
        ...
        let merData = data.merchant;
        let intoData = data.plantMchtInto;
        ...
        //数据导入需要时间，放在后面，防止函数中途throw error
