    ...
    <el-form ref="addForm" :model="addForm" :label-width="labelWidth" size="small">
      ...
      <el-card class="box-card" style="margin-top: 14px">
        ...
        <el-form-item label="支付宝账号：">
          ...
          <span class="tips">企业/法人/非法人支付宝账号</span>
        </el-form-item>
  ...
  export default {
    ...
    methods: {
      ...
      modifyMerchantInfo: function (id) {
        // 编辑模式
        getAlipayInfo(id, this.payChannel).then(res => {
          this.autoInputData(res.obj)
        }).catch(e => {
      ...
      submitAddData: function (form) {
        ...
        modifyMerchant(JSON.stringify(form), this.payChannel).then(res => {
      ...
      autoInputData: function (row) {
        ...
        if (row.plantMchtInto) {
          ...
          this.addForm.aliPayAccount = data.aliPayAccount || ''; //支付宝账号
      ...
      downloadFile: function (type) {
        if (type == 'localCode') {
          window.location.href = 'http://www.lianhanghao.com/'
