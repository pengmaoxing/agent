    ...
    <el-form :model="form" ref="form" :rules="rules" :label-width="labelWidth" :size="inputSize">
        ...
        <div>
          ...
          <el-form-item label="渠道：" prop="pay_type">
            <el-select v-model="form.pay_type" placeholder="渠道">
              <el-option value="cloudpay" label="微信买单"></el-option>
            </el-select>
          </el-form-item>
      ...
      <el-card class="box-card" style="margin-top: 14px">
        ...
        <div class="btn-group" v-if="mode==='verify'">
          ...
          <el-button type="primary" @click="dataCollation" :loading="btnLoading">微信买单审核通过</el-button>
...
<script>
  ...
  import {returnThumbnailUrl, uploadUrl, wxmduploadUrl, returnFilesUrl} from '@/modules/file/api/upload' //图片操作API
  ...
  export default {
    ...
    methods: {
      ...
      newAddMerchantInfo: function (id) {
        //新进件模式
        getWxmdInfo(id, this.payChannel).then(res => {
          let data = res.obj;
   ...
   //     this.form.shop_phone = data.customerPhone || '';
        ...
        this.form.pay_type = data.pay_type || 'cloudpay'; //渠道 lb
